# Contributing to OrionBeacon

Thanks for taking the time to look at this. Leader election for .NET: a renewable-lease elector with fencing tokens, a hosted background loop that keeps one instance elected, and IsLeader state plus leadership-change events. The project is small and the bar for contributions is "does it make the package clearer, faster, or safer without expanding the public surface needlessly."
...
```bash
git clone https://github.com/tunahanaliozturk/OrionBeacon
cd OrionBeacon
dotnet restore
